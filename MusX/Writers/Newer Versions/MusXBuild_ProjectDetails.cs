...
namespace MusX.Writers
{
    ...
    {
        //-------------------------------------------------------------------------------------------------------------------------------
        public static void BuildProjectDetailsFile(string projDataFilePath, string OutputFilePath, string platform, uint fileHashCode, bool bigEndian, int fileVersion)
        {
            ...
            if (!string.IsNullOrEmpty(OutputFilePath))
            {
                ...
                using (BinaryWriter binWriter = new BinaryWriter(File.Open(OutputFilePath, FileMode.Create, FileAccess.Write, FileShare.Read), Encoding.ASCII))
                {
                    ...
                    uint projDataStart = BytesFunctions.FlipData(positionAligned, bigEndian);
                    ...
                    if (File.Exists(projDataFilePath))
                    {
                        ...
                        {
                            projDataLength = BytesFunctions.FlipData((uint)sfxFileData.Length, bigEndian);
