...
namespace MusX.Writers
{
    ...
    {
        //-------------------------------------------------------------------------------------------------------------------------------
        public static void BuildMusicFile(string mkrFilePath, string ssdFilePath, string OutputFilePath, string platform, uint fileHashCode, int fileVersion)
        {
            ...
            if (!string.IsNullOrEmpty(OutputFilePath))
            {
                ...
                using (BinaryWriter binWriter = new BinaryWriter(File.Open(OutputFilePath, FileMode.Create, FileAccess.Write, FileShare.Read), Encoding.ASCII))
                {
                    ...
                    binWriter.Write(BytesFunctions.FlipData(soundMarkerFileStart, isBigEndian));
                    binWriter.Write(BytesFunctions.FlipData(soundMarkerFileLength, isBigEndian));

                    //File length 2
                    binWriter.Write(BytesFunctions.FlipData(soundSampleDataStart, isBigEndian));
                    binWriter.Write(BytesFunctions.FlipData(soundSampleDataLength, isBigEndian));
                }
            }
        }
    }
