...
namespace MusX.Writers
{
    ...
    {
        //-------------------------------------------------------------------------------------------------------------------------------
        public static void BuildStreamFile(string binFilePath, string lutFilePath, string outputFilePath, uint fileHashCode, string platform, bool isBigEndian, int fileVersion)
        {
            ...
            if (!string.IsNullOrEmpty(outputFilePath))
            {
                ...
                using (BinaryWriter binWriter = new BinaryWriter(File.Open(outputFilePath, FileMode.Create, FileAccess.Write, FileShare.Read), Encoding.ASCII))
                {
                    ...
                    if (File.Exists(lutFilePath))
                    {
                        ...
                        //Write data
                        BytesFunctions.WriteAlignedDecoration(binWriter, positionAligned);
                    ...
                    binWriter.Write(BytesFunctions.FlipData(lutFileDataStart, isBigEndian));
                    binWriter.Write(BytesFunctions.FlipData(lutFileDataLength, isBigEndian));

                    //File length 2
                    binWriter.Write(BytesFunctions.FlipData(binFileDataStart, isBigEndian));
                    binWriter.Write(BytesFunctions.FlipData(binFileDataLength, isBigEndian));
                }
            }
        }
    }
