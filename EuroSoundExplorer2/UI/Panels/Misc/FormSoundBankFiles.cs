    ...
    public partial class FormSoundBankFiles : DockContent
    {
        ...
        public FrmMain MainForm
        {
            get
            {
                ...
                return (FrmMain)Application.OpenForms[nameof(FrmMain)];
            }
            set { mainForm = value; }
        }

        private AppConfig Configuration { get { return MainForm.Configuration; } }
        private HashcodeParser HashTable { get { return MainForm.HashTable; } }
        private string SelectedPlatformName { get { return Configuration.PlatformSelected.ToString(); } }
        private string ProjectFolder { get { return Configuration.ProjectFolder; } }
        ...
        private sealed class ProjectGroupExportResult
        {
            public EuroSoundGroupExportResult ExportResult { get; set; }
            public int SoundbankCount { get; set; }
            public int FailedCount { get; set; }
            public bool Cancelled { get; set; }
            public List<string> FailedFiles { get; set; }

            public ProjectGroupExportResult()
            {
                ExportResult = new EuroSoundGroupExportResult();
                FailedFiles = new List<string>();
            }
        }
        ...
        {
            HashcodeParser hashTable = HashTable;
            Platform selectedPlatform = Configuration.PlatformSelected;
        ...
        private void TreeView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                //Get folder path
                string parentFolder = ProjectFolder;
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        private void MenuItem_ExportProjectGroupsToEuroSoundGroups_Click(object sender, EventArgs e)
        {
            ExportProjectGroupsToEuroSoundGroups();
        }
        ...
        public void ExportSfxSamplesToEuroSoundFile(IEnumerable<Sample> samples)
        {
            ...
            try
            {
                int fileVersion = SoundBankHeaderData.FileVersion != 0 ? SoundBankHeaderData.FileVersion : Configuration.FileVersion;
                Dictionary<uint, EuroSoundSfxRadiusData> radii = BuildSoundDetailsRadiusLookup(SoundDetails);
                EuroSoundSfxTextExportResult result = EuroSoundSfxTextExporter.Export(samplesToExport, fileVersion, HashTable, outputFolder, sections, radii);
        ...
        private void ExportProjectSfxSamplesToEuroSoundFiles()
        {
            ...
            ProjectSfxExportOptions options = new ProjectSfxExportOptions
            {
                ...
                SelectedTitle = Configuration.TitleSelected,
                Hashcodes = HashTable,
                Sections = sections
            };
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        private void ExportProjectGroupsToEuroSoundGroups()
        {
            string projectFolder = ProjectFolder;
            if (!Directory.Exists(projectFolder))
            {
            ...
            if (!TrySelectEuroSoundGroupsFolder(out string groupsFolder))
            {
                return;
            }
            ...
            using (FrmProgress progressForm = new FrmProgress(
                "Export Project Groups to EuroSound Groups",
                "Preparing group export...",
                worker => ExportProjectGroupsWorker(options, worker)))
            {
                progressForm.ShowDialog(this);
                ...
                ProjectGroupExportResult result = (ProjectGroupExportResult)progressForm.Result;
                if (result == null)
                {
                    return;
                }
                ...
                if (result.Cancelled)
                {
                    MessageBox.Show("Project group export was cancelled.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                ShowEuroSoundGroupExportResult(result.ExportResult, result.SoundbankCount, result.FailedCount, result.FailedFiles);
            }
        }
        ...
        private ProjectSfxExportResult ExportProjectSfxSamplesWorker(ProjectSfxExportOptions options, BackgroundWorker worker)
        {
            ...
            string[] files = EuroSoundProjectSoundBankLoader.GetProjectSfxFiles(options.ProjectFolder);
            ...
            EuroSoundProjectSoundBankLoadResult loadResult = EuroSoundProjectSoundBankLoader.LoadSoundBanks(
                CreateSoundBankLoadOptions(options),
                worker,
                90,
                "Inspecting");

            totalResult.Cancelled = loadResult.Cancelled;
            totalResult.FailedFiles.AddRange(loadResult.FailedFiles);
            totalResult.FailedCount += loadResult.FailedCount;
            if (totalResult.Cancelled)
            {
                return totalResult;
            }

            foreach (EuroSoundProjectSoundBank soundBank in loadResult.SoundBanks)
            {
                try
                {
                    worker.ReportProgress(92, "Exporting " + Path.GetFileName(soundBank.FilePath));
                    EuroSoundSfxTextExportResult fileResult = EuroSoundSfxTextExporter.Export(soundBank.Samples.Values, soundBank.Header.FileVersion, options.Hashcodes, options.OutputFolder, options.Sections, radii);
                ...
                {
                    totalResult.FailedFiles.Add(Path.GetFileName(soundBank.FilePath) + ": " + ex.Message);
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        private ProjectGroupExportResult ExportProjectGroupsWorker(ProjectSfxExportOptions options, BackgroundWorker worker)
        {
            ProjectGroupExportResult result = new ProjectGroupExportResult();
            ...
            result.Cancelled = loadResult.Cancelled;
            result.FailedFiles.AddRange(loadResult.FailedFiles);
            ...
            foreach (EuroSoundProjectSoundBank soundBank in loadResult.SoundBanks)
            {
                allSamples.AddRange(soundBank.Samples.Values);
                result.SoundbankCount++;
            }

            if (allSamples.Count > 0)
            {
                worker.ReportProgress(95, "Updating group files...");
                result.ExportResult = EuroSoundGroupExporter.Export(allSamples, options.Hashcodes, options.OutputFolder);
            }
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        private EuroSoundProjectSoundBankLoadOptions CreateSoundBankLoadOptions(ProjectSfxExportOptions options)
        {
            return new EuroSoundProjectSoundBankLoadOptions
            {
        ...
        public bool TryGetSoundDetailsRadius(uint hashCode, out EuroSoundSfxRadiusData radiusData)
        {
            ...
            string cacheKey = projectFolder + "|" + Configuration.PlatformSelected + "|" + Configuration.TitleSelected;
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        private bool TrySelectEuroSoundGroupsFolder(out string groupsFolder)
        {
            groupsFolder = string.Empty;

            using (FolderBrowserDialog folderBrowserDialog = new FolderBrowserDialog())
            {
                folderBrowserDialog.Description = "Select the EuroSound project groups folder to update.";
                if (folderBrowserDialog.ShowDialog(this) != DialogResult.OK)
                {
                    return false;
                }

                groupsFolder = folderBrowserDialog.SelectedPath;
                return true;
            }
        }
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        private void ShowEuroSoundGroupExportResult(EuroSoundGroupExportResult result, int soundbankCount, int failedCount, List<string> failedFiles)
        {
            string message = string.Format(
                "Recovered {0} groups.\r\nUpdated: {1}\r\nSoundBanks: {2}",
                result.RecoveredGroupCount,
                result.UpdatedGroupCount,
                soundbankCount);

            AppendGroupExportWarnings(ref message, "Groups not found", result.UnmatchedGroups);
            AppendGroupExportWarnings(ref message, "Ambiguous groups", result.AmbiguousGroups);
            AppendGroupExportWarnings(ref message, "Conflicting recovered groups", result.ConflictingGroups);
            AppendGroupExportWarnings(ref message, "SFX names not found in Sound.h", result.MissingSfxNames);
            ...
            bool hasWarnings = failedCount > 0 ||
                result.UnmatchedGroups.Count > 0 ||
                result.AmbiguousGroups.Count > 0 ||
                result.ConflictingGroups.Count > 0 ||
                result.MissingSfxNames.Count > 0;

            MessageBox.Show(message, Application.ProductName, MessageBoxButtons.OK, hasWarnings ? MessageBoxIcon.Warning : MessageBoxIcon.Information);
        }

        //-------------------------------------------------------------------------------------------------------------------------------
        private void AppendGroupExportWarnings(ref string message, string title, List<string> warnings)
        {
            ...
            message += string.Format("\r\n{0}: {1}", title, warnings.Count);
            int maxWarningsToShow = Math.Min(warnings.Count, 5);
        ...
        private void LoadSelectedSfx(string filePath)
        {
            ...
            EuroSoundFileLoader.LoadSoundBank(CreateFileLoadOptions(filePath), LoadedData);

            //Display Data
            MainForm.pnlSbHashCodes.SetHashCodesToListView();
            MainForm.pnlWavHeaderData.ShowWavesList();
        }

        //-------------------------------------------------------------------------------------------------------------------------------
        private void LoadSelectedStream(string filePath)
        {
            EuroSoundFileLoadResult result = EuroSoundFileLoader.LoadStreamBank(CreateFileLoadOptions(filePath), LoadedData);
            foreach (string loadedFilePath in result.LoadedFilePaths)
            {
                UpdateListViewLoadedState(loadedFilePath, "Loaded");
            }

            //Display Data
            MainForm.pnlStreamData.ShowStreamData();
        }
        ...
        private void LoadSelectedMusic(string filePath)
        {
            ...
            EuroSoundFileLoader.LoadMusicBank(CreateFileLoadOptions(filePath), LoadedData);

            //Display Data
            MainForm.pnlMusicData.ShowMusicData();
        }
        ...
        private void LoadSelectedSbi(string filePath)
        {
            ...
            EuroSoundFileLoader.LoadSbi(CreateFileLoadOptions(filePath), LoadedData);
        ...
        private void LoadSelectedProjectDetails(string filePath)
        {
            ...
            EuroSoundFileLoader.LoadProjectDetails(CreateFileLoadOptions(filePath), LoadedData);

            MainForm.pnlProjDetailsMemSlots.ShowData();
            MainForm.pnlProjDetailsSoundBanks.ShowData();
            MainForm.pnlProjDetailsData.ShowData();
        }
        ...
        public void LoadSelectedSoundDetails(string filePath)
        {
            ...
            EuroSoundFileLoader.LoadSoundDetails(CreateFileLoadOptions(filePath), LoadedData);

            MainForm.pnlSoundDetailsData.ShowData();
        }
        ...
        public void LoadSelectedMusicDetails(string filePath)
        {
            ...
            EuroSoundFileLoader.LoadMusicDetails(CreateFileLoadOptions(filePath), LoadedData);

            MainForm.pnlMusicDetailsData.ShowData();
        }
        ...
        public void LoadSelectedMusicMarkers(string filePath)
        {
            ...
            EuroSoundFileLoader.LoadMusicMarkers(CreateFileLoadOptions(filePath), LoadedData);

            MainForm.pnlMusicMarkersData.ShowData();
        }

        //-------------------------------------------------------------------------------------------------------------------------------
        private EuroSoundFileLoadOptions CreateFileLoadOptions(string filePath)
        {
            return new EuroSoundFileLoadOptions
            {
                FilePath = filePath,
                ProjectFolder = ProjectFolder,
                Platform = SelectedPlatformName
